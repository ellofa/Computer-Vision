...
def train(
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data = data.view(data.shape[0], 3, 224, 224)
        if use_cuda:
...
def main():
    ...
    print("end train_loader creation")
    val_loader = torch.utils.data.DataLoader(
    ...
    # Learning Scheduler 
    scheduler  = CosineAnnealingLR(model.optimizer, T_max=200)
    
    # Loop over the epochs
    best_val_loss = 1e8
    wandb.init(
    # set the wandb project where this run will be logged
    project="RecVis_Challenge",
    
    # track hyperparameters and run metadata
    config={
    "architecture": "Vit",
    "transform" : "Aug_trans",
    "dataset": "classifysketch",
    "epochs": 15,
    }
)
    for epoch in range(1, args.epochs + 1):
        # training loop
        train(model, scheduler, train_loader, use_cuda, epoch, args)
        # validation loop
        ...
        model_file = "/content/drive/My Drive/recvis23_a3/models" + "/model_" + str(epoch) + ".pth"
